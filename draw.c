...
void
drawtext(DC* dc, const char* text, long unsigned int col[3], int yAxis) {
	char buf[256];
	size_t n, mn;
	...
	drawrect(dc, 0, yAxis, dc->w, dc->h, True, BG(dc, col));
	drawtextn(dc, buf, mn, col, yAxis);
}

void
drawtextn(DC *dc, const char *text, size_t n, unsigned long col[ColLast], int yAxis) {
	int x, y;
	...
	if(dc->font.set)
		XmbDrawString(dc->dpy, dc->canvas, dc->font.set, dc->gc, x, y + yAxis, text, n);
	else {
		XSetFont(dc->dpy, dc->gc, dc->font.xfont->fid);
		XDrawString(dc->dpy, dc->canvas, dc->gc, x, y + yAxis, text, n);
	}
}
