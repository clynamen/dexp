...
static void scanCurDir(void);
//scan child directory and save contenent in a string
static void scanChiDir(void);
...
drawmenu(void) {
	...
	drawrect(dc, 0, yMid, mw, mh, True, BG(dc, normcol));
	drawrect(dc, 0, yBot, mw, mh, True, BG(dc, normcol));
	...
	if(prompt) {
		dc->w = promptw;
		drawtext(dc, prompt, selcol, yMid);
		dc->x = dc->w;
	}
	dc->w = (lines > 0 || !matches) ? mw - dc->x : inputw;
	drawtext(dc, text, normcol, yMid);
	
	
	if((curpos = textnw(dc, text, cursor) + dc->h/2 - 2) < dc->w)
		drawrect(dc, curpos, yMid, 1, dc->h - 4, True, FG(dc, normcol));

	if(lines > 0) {
		dc->w = mw - dc->x;
		for(item = curr; item != next; item = item->right) {
			dc->y += dc->h;
			drawtext(dc, item->text, (item == sel) ? selcol : normcol, yMid);
		}
	}
	else if(matches) {
		dc->x += inputw;
...
setup(void) {
	...
	win = XCreateWindow(dc->dpy, root, x, y, mw, mh*3+2, 0,
	                    DefaultDepth(dc->dpy, screen), CopyFromParent,
	                    DefaultVisual(dc->dpy, screen),
	                    CWOverrideRedirect | CWBackPixmap | CWEventMask, &wa);

	grabkeyboard();
	resizedc(dc, mw, mh*3+2);
...
scanCurDir(void) {
	...
	if (!(dir=opendir(curDir)))
	  ...
	  for(end = &items; (info=readdir(dir)); *end = item, end = &item->next) {
// 		if((p = strchr(info->d_name, '\n')))
// 			*p = '\0';
		if(!(item = malloc(sizeof *item)))
			eprintf("cannot malloc %u bytes\n", sizeof *item);
		if(!(item->text = strdup(info->d_name)))
